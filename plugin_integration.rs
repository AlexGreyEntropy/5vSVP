...
pub fn integrate_plugin_with_collection(
    ...
    let cpi_accounts = AddPluginV1CpiBuilder::new(&ctx.accounts.collection)
        ...
        // Can add the Post-Quantum signature here
        // Can add the GOAT SDK plugin bellow
        pub fn universal_goat_integration(ctx: Context<UniversalIntegration>, integration_type: IntegrationType, params: String) -> Result<()> {
            match integration_type {
                IntegrationType::Tool => goat_tool_integration(&params),
                IntegrationType::Wallet => goat_wallet_integration(&params),
                IntegrationType::Agent => goat_agent_integration(&params),
                IntegrationType::Blockchain => goat_blockchain_integration(&params),
            }
        }
...
pub fn integrate_plugin_with_asset(
    ...
    let cpi_accounts = AddPluginV1CpiBuilder::new(&ctx.accounts.asset)
        .with_update_authority(&ctx.accounts.update_authority)
        .with_plugin(Plugin { address: plugin_address });
        // Or add the Post-Quantum signature here
        // can add the GOAT SDK plugin bellow
        pub fn universal_goat_integration(ctx: Context<UniversalIntegration>, integration_type: IntegrationType, params: String) -> Result<()> {
            match integration_type {
                IntegrationType::Tool => goat_tool_integration(&params),
...
// initialize a vault for an NFT when it's minted
...
// can add ddditional helper functions or CPI calls for plugin management